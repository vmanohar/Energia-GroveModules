...
void SeeedGrayOLED::putChar(unsigned char Cee)
{
    if(Cee < 32 || Cee > 127) //Ignore non-printable ASCII characters. This can be modified for multilingual font.
    {
        Cee=' '; //Space
    }	
    ...
    for(char i=0;i<8;i=i+2)
    {
        for(char j=0;j<8;j++)
        {
            ...
            char c=0x00;
            char bit1=(pgm_read_byte(&BasicFont[Cee-32][i]) >> j)  & 0x01;  
            char bit2=(pgm_read_byte(&BasicFont[Cee-32][i+1]) >> j) & 0x01;
